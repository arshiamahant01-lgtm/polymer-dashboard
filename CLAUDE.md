...
|---|---|---|---|
| `fetch_macro.py` | EIA API v2 (Brent/WTI, Henry Hub) + Business Insider markets pages (same, fresher), World Bank API (GDP), Frankfurter.dev (USD/INR) | EIA: yes, free (`EIA_API_KEY` env var). Others: no | FRED CSV endpoint is bot-blocked from scripted `curl`/`requests` (connection reset) — do not use it. EIA's own daily spot-price series lags real-time by 1-2+ weeks in practice, so it's used only for its ~1-year backfill depth. Business Insider (`markets.businessinsider.com/commodities/{oil-price, oil-price?type=wti, natural-gas-price}`) has no key/auth and embeds a same-day-updated ~3-week rolling daily-OHLC table as inline JSON (`window.historicalPrices.configs` — found via view-source, same JS-bundle-discovery approach as Credco) and is fetched *after* EIA so its fresher values win on overlapping dates. No naphtha equivalent exists there (checked, 404) — naphtha's crack-spread proxy re-derives from `crude_oil.csv` on every run, so it freshens automatically once Brent does. |
